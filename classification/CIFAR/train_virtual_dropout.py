...
# Checkpoints
parser.add_argument('--save', '-s', type=str, default='./snapshots/dropout_uniform', help='Folder to save checkpoints.')
...
# choose dropout method
parser.add_argument('--dropout_methods', '-bdm', type=str, default='uniform')
...
if args.dataset == 'cifar10':
    train_data = dset.CIFAR10('/data/coq20tz/CIFAR', train=True, transform=train_transform, download=True)
    test_data = dset.CIFAR10('/data/coq20tz/CIFAR', train=False, transform=test_transform, download=True)
    num_classes = 10
else:
    train_data = dset.CIFAR100('/data/coq20tz/CIFAR', train=True, transform=train_transform, download=True)
    test_data = dset.CIFAR100('/data/coq20tz/CIFAR', train=False, transform=test_transform, download=True)
    num_classes = 100
...
def log_sum_exp(value, dim=None, keepdim=False):
    ...
    if dim is not None:
        ...
        m = torch.max(value)
        sumexp_exp = torch.sum(torch.exp(value - m))
